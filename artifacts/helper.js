...
function getAccessToken(clientId, clientSecret, callback) {
    request
        .post({
            ...
            // url:'https://westus.api.cognitive.microsoft.com/speech/v0/internalIssueToken HTTP/1.1',
...
function textToSpeech(text, filename, accessToken, callback) {
    var ssmlTemplate = "<speak version='1.0' xml:lang='en-us'><voice xml:lang='%s' xml:gender='%s' name=" +
            "'%s'>%s</voice></speak>";
    request.post({
        ...
        if (err) 
            return callback(err);
        fs
            .writeFile(filename, body, 'binary', function (err) {
                if (err) 
                    return callback(err);
                callback(null);
            });
...
function speechToText(filename, accessToken, callback) {
    fs
        .readFile(filename, function (err, waveData) {
            if (err) 
                return callback(err);
            request
                .post({
